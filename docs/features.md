...
| Morning gap-down over-reaction/recovery study | confirmed | The first strategy; uses EOD open plus direct hourly checkpoints for a coarse pass, and 5-minute bars for the complete opening-window study (D-012). D-026 makes eligibility local to the contiguous lookback through each decision timestamp: remote history gaps are irrelevant, and missing future checkpoints remain selected events with explicit outcome status rather than lookahead exclusions |
...
| Research price/causality and sample audit | confirmed | D-036/M3–M4: consistent price bases, as-of features and perturbation tests, coverage funnels, missing-outcome sensitivity, and market/regime controls; research-protocol.md defines acceptance |
