...
| EOD daily backfill + nightly incremental update (resumable, idempotent) | confirmed | Instrument-owned canonical primitives and exact-dataset request validation are built and tested; production use awaits M1's controlled Tiingo canary |
| Intraday ingestion: 1-hour **and 5-minute** bars | confirmed | Storage/query and exact-frequency identity validation are built; production use awaits M1's controlled Tiingo canary, and historical intraday also needs client hardening for RE-002/RE-004. Measured history begins 2016-12-12; direct hourly omits the opening half-hour (intraday-spike.md, D-012) |
...
| Tiingo request-budget / bandwidth tracking with budget-aware backfill scheduling | confirmed | Required by D-011/D-013/D-020: track all request/byte caps, refresh current data first, resume durable breadth-first sweeps after quota stops, hard-cap 5-minute history at 30 GB/month, and preserve 10 GB for current/ongoing work. Bulk fetches use `format=csv` (client switch landed in M1; scheduler/accounting lands in M2) |
