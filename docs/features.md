...
| Phased backfill: seed EOD 20y + 1-hour from 2016-12-12, then all-ticker EOD 20y, then seed 5-minute newest-to-oldest from current to 2016-12-12; current all-ticker collection continues daily | confirmed | Scope and phase priority in D-011; D-020 makes every historical dataset breadth-first by request depth, while D-025 admits history against a 30 GB total-usage ceiling that rises daily to 39 GB over the final seven UTC calendar days. The validated phase-1 EOD pass, post-backfill identity repair, and exact-hourly identity bootstrap ran on 2026-08-28. Per D-026, terminal ranges remain durable exclusions, release later phase gates, and require explicit `--retry-blocked` intent rather than routine timer reactivation |
...
| Tiingo request-budget / bandwidth tracking with budget-aware backfill scheduling | confirmed | Built in M2 per D-011/D-013/D-020/D-025: durable pre-request reservations and encoded-byte settlement track all attempts; a conservative rolling window enforces a 30-to-39 GB late-month historical admission ramp against total usage plus the 40 GB current-work hard ceiling. Current-first cycles, phase gates, immutable stable-instrument cohorts, and persisted breadth-first cursors make quota stops resumable. Bulk fetches use `format=csv` |
