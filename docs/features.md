...
| Ongoing active-universe EOD + rolling top-liquidity intraday collection | confirmed | Built in schema v8 per D-030–D-033: each post-market cycle freezes active supported listings and bridges EOD from stored coverage; a persisted monthly top-5,000 cohort uses the latest 20 completed sessions of canonical EOD `close * volume`, after which independent current-mode hourly and five-minute sweeps bridge existing coverage or start new members forward-only. Missing current-session bars and cancellations are cycle-scoped exclusions, not global stalls; forward-only floors cannot creep backward. Bounded jobs yield the lock, share current-work budgets, expose durable status, and refuse automatic daytime runs. The live database is schema v8, but production program initialization/timer cutover and duty-cycle validation remain |
...
| Scheduled ongoing updates | confirmed | The target server still has the interim latest-seed-universe EOD timer; its editable install already migrated the live metadata database to schema v8. The replacement `market-data-ongoing` service/timer is implemented: it starts at 23:30 UTC weekdays, paces 1,000-turn API batches by six minutes and zero-request transitions by one second, completes or checkpoints before 08:00 New York time, writes bounded status, reserves accepted exit 3 for explicit terminal exclusions, and retries exit-1/2 failures. It must pass the human commit gate, be initialized, and replace the interim timer before this is live. External notification remains optional |
...
| Tagged-ticker morning decision feed | proposed | D-031: a later small owner-maintained watchlist may receive realtime or five-minute morning updates for triggering decisions, from Tiingo or a broker's read-only market-data API. It is separate from M4's overnight collector and from canonical Tiingo bars, and requires its own promotion decision |
| General realtime/streaming layer | proposed | Triage 2026-08-26: owner deliberately keeps this proposed; revisit after M3. D-031 narrows the likely first use to the tagged morning watchlist; research-only and no order execution per D-007 |
