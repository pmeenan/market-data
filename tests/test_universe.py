...
def _synthetic_eod(ticker: str, close: float, volume: int, days: int = 100):
    ...
    return pl.DataFrame({
        ...
        "volume": [volume] * days,
        "adj_open": [close] * days, "adj_high": [close] * days,
        "adj_low": [close] * days, "adj_close": [close] * days,
        "adj_volume": [volume] * days,
        "div_cash": [0.0] * days, "split_factor": [1.0] * days,
    }).cast(EOD_SCHEMA)
...
def test_rank_by_dollar_volume(tmp_path):
    ...
    bars.write_eod("BRIEF", _synthetic_eod("BRIEF", close=200.0, volume=2_000_000, days=10))
...
def test_import_real_seed(tmp_path):
    ...
    seed = Path(__file__).resolve().parent.parent / "seeds" / "universe_by_dollar_volume.csv"
    if not seed.exists():
        import pytest
